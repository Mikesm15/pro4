...
public class InfantFrame extends JFrame
{
     ...
     * @version 2017-11-1
     * 
     *          Selection panel: contains JLists for the list of trials, the list of
     *          fieldNames and the list of subfieldNames. Note that the displayed
     *          subfieldNames is dependent on which field has been selected
     *
     */
    private class SelectionPanel extends JPanel
    {
         ...
         * Constructor
         * 
         * Creates a 3x2 grid of components with labels down the left column and Jlist
         * down the 2nd column
         */
        ...
        /**
         * Update subfield selection list based on the subfields available for the
     ...
     * 
     * Two frames are side-by-side: SelectionPanel on the left and DataPanel on the
     * right.
     */
    public InfantFrame()
    {
     ...
     * Load a new Infant.
     * 
     * This method is declared as "synchronized" to ensure that only one thread is
     * allowed to call it and other synchronized methods at once.
     * 
     * @param directory
     ...
     * allowed to call it and other synchronized methods at once.
     * 
     * One of the challenges in implementing this method is that it can be called at
     * any time. In particular, the structures that it is referencing may be in the
     * process of being updated. So, we cannot assume that things like "selected
     * values" are not actually set to something interesting or useful.
     */
    public synchronized void update()
    {
        ...
        {
            // Create an Infant object with only the selected weeks to the new Infant
            // constructor
            ...
            if (fieldName != null && !fieldName.equals(""))
            {
                ...
                if (subfieldName != null && !subfieldName.equals(""))
                {
                    ...
                    maxStateTimeString = subInfant.getMaxState(fieldName, subfieldName).toString();
                    // What is the min state?
                    minStateString = subInfant.getMinState(fieldName, subfieldName).toString();
                    // What is the min state week?
                    minStateWeekString = subInfant.getMaxState(fieldName, subfieldName).toString();
                    // What is the min state time?
                    minStateTimeString = subInfant.getMaxState(fieldName, subfieldName).toString();
                    // What is the average?
