...
/**
 * The application frame. This class contains the main method, creating an
 * instance of itself and running the application.
 * 
 * @author Stephen
 * @version 2017-11-15
 */
public class SierpinskiApplication
{
    ...
    /**
     * The SierpinskiPanel instance responsible for maintaining and computing the
     * state of the spinner as well as drawing it.
     */
    ...
    /**
     * Check box specifying whether or not the spinners should be drawn filled in.
     */
    private JCheckBox wireframeCheckBox;

    /**
     * Check box specifying whether or not to draw inscribed circles on the
     * triangles.
     */
    private JCheckBox circleCheckBox;

    /**
     * Check box specifying whether or not to draw triangles inscribing the circles.
     */
    ...
    public SierpinskiApplication(String title, int width, int height)
    {
        ...
        sizeSlider = new JSlider(JSlider.HORIZONTAL, height / 4, height, height / 4);
        ...
        controlPanel.setBorder(BorderFactory.createTitledBorder("Display Parameters"));
        ...
        controlPanel.addControl("Radius", sizeSlider, "Changes the radius of the spinners.");

        controlPanel.addControl("Wireframe", wireframeCheckBox,
                "Specifies whether or not to draw only the outlines " + "of the spinners.");

        controlPanel.addControl("Draw circles", circleCheckBox,
                "Specifies whether or not to draw inscribed circles on" + "the triangles.");

        controlPanel.addControl("Draw triangles", triangleCheckBox,
                "Specifies whether or not to draw triangles around" + "the circles.");

        contentPanel = new JPanel(new GridBagLayout());

        JPanel chosenColorsPanel = new JPanel(new GridBagLayout());
        chosenColorsPanel.setBorder(BorderFactory.createTitledBorder("Current colors"));
        ...
        sPanel.addMouseListener(new MouseAdapter()
        {
            ...
            {
                /*
                 * We change the spinner displayed in the SierpinskiPanel, grabbing the current
                 * values of our property controls to pass into the SierpinskSpinner
                 * constructor.
                 */
                sPanel.changeTriangle(new SierpinskiSpinner(recursionDepthSlider.getValue(), event.getPoint(),
                        sizeSlider.getValue(), rotationSlider.getValue() * (Math.PI / 180), Color.WHITE, Color.WHITE,
                        !wireframeCheckBox.isSelected(), circleCheckBox.isSelected(), triangleCheckBox.isSelected()));
            }
        });
        ...
        rotationSlider.addChangeListener(new ChangeListener()
        {
            ...
            public void stateChanged(ChangeEvent event)
            {
                sPanel.setRotationalVelocity(rotationSlider.getValue() * (Math.PI / 180));
            }
        });
         ...
         * 
         * Initialize the timer variable to a newly created timer that fires at a
         * regular interval, calling a method in SierpinskiPanel causing the next step
         * of animation to be computed, and the screen to be re-drawn.
         */
        ...
        timer = new Timer(ANIMATION_TICK, taskPerformer);
    }
    ...
    {
        new SierpinskiApplication("Sierpinski Spinners", 1280, 768).start();
    }
}
